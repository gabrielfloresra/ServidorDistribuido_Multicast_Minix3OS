    ...
    else
    {
      if ( !strcmp(destino, localIP) )
      {
        if( strstr("|+|", mensaje) != NULL ){
          ...
          numAssistanceList = atoi(strtok(mensaje,"|+|"));
          printf(ANSI_COLOR_BLUE "=>peticion para escribir recibida, casoTimer: %d\n" ANSI_COLOR_RESET, casoTimer);
          write_toFile(strtok(NULL,"|+|"));
          printf(ANSI_COLOR_GREEN "=>liberando\n" ANSI_COLOR_RESET);
        } else {
...
void addPeer()
{
  ...
  while (1)
  {
    ...
    if( !strcmp(peticion, MSJ_REQUEST_TO_JOIN) && strcmp(ipOrigen, localIP)){
      ...
      snprintf( mensaje, TAM_MAX_CAD, "%s::%s::%s::%d|+|bloqueado", localIP, ipOrigen, MSJ_REQUEST_TO_JOIN, num);
      sendMsj( mensaje);
      ...
      if ((procesoEscritura = fork()) < 0) {
        printf("error al crear el proceso de escritura: \n");
        terminarPrograma();
      }
      if (procesoEscritura){
